...
<div class="max-w-4xl mx-auto p-6 space-y-6 fade-in-up">
    ...
    <div class="modern-card p-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit User: {{ $user->name }}</h1>
    ...
    <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="modern-card p-6" id="professional-section" style="{{ $user->role === 'chauffeur' ? 'display: block;' : 'display: none;' }}">
            <h2 class="text-xl font-semibold text-gray-800 mb-6 flex items-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" 
                           name="license_expiry" 
                           id="license_expiry"
                           value="{{ old('license_expiry', $user->license_expiry ? $user->license_expiry->format('Y-m-d') : '') }}"
                           min="{{ date('Y-m-d') }}"
        ...
        <div class="modern-card p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="password" class="block text-sm font-semibold text-gray-700 mb-2">New Password (leave blank to keep current)</label>
                    <input type="password" 
                           name="password" 
                           id="password"
                           minlength="8"
